document.addEventListener("DOMContentLoaded", function () {
  ...
  function createButtons() {
    ...
    specialButton.style.left = "8%"; // Ajusta la posición X
    ...
    specialButton.addEventListener("click", () => {
      ...
      observer.observe(special, config);

      special.classList.add("active");
      special.innerHTML =
        "<strong style='text-align: center;'>First letter, from more to less.</strong> <br><br>" +
        "A voice whispers and behind the note, the sudden text appears: <br>" +
        "<a href='https://morsecode.world/international/translator.html'>Language of the war</a><br><br>" +
        ...
        "<br> Make sure to chose 10 as base and to convert it to the lower case Q <br><br>" +
  ...
  function clickRegularButtons(index) {
    if (index === buttonNotFillingIndex) {
      ...
      addSearchLog(places[index].name, false);
  ...
  function handleSearch(index) {
    ...
    if (availableBoxes.length > 0) {
      ...
      addSearchLog(places[index].name, true);
    } else {
      alert("ERROR- please contact fallen with the code Morse-HandleSearch");
    }
  }
});
...
function checkFinalResult(value) {
  // document.getElementById("text").innerHTML = "";
  // console.log("Checking final result for value:", value); // Debugging line
  if (value === "1025") {
    // Es correcto, pon ok
    document.getElementById("result").style.display = "block";
    // document.getElementById("result").style.background = "green";

    document.getElementById("result").innerHTML = 'A beep is heard and the screen turns green <a href="./LORE/Morse.html">and you can go throught.</a>.';
  } else {
...
function addSearchLog(placeName, encontrado) {
  let searchBox = document.getElementById("search-log-list");
  //crear un div
  let divCreado = document.createElement("div");
  divCreado.classList.add("search-log-item", "mb-2");

  divCreado.innerText = `${placeName}:` + (encontrado ? " Found something" : " Nothing found");
  searchBox.appendChild(divCreado);
  // console.log("Adding to search log:", placeName, encontrado); // Debugging line
}
