document.addEventListener("DOMContentLoaded", function () {
  ...
  function createLightSwitch(destroy) {
    ...
    button.classList.add("btn", "botonesConst", "btn-lg");
...
document.querySelectorAll('button.btn').forEach(button => {
  button.style.setProperty('background', '#5a0e0e', 'important');
  button.style.setProperty('color', '#f0d9a0', 'important');
});
document.querySelectorAll('button.botonesConst').forEach(input => {
    input.style.setProperty('background', '#5a0e0e', 'important');
    input.style.setProperty('color', '#f0d9a0', 'important');
});
document.querySelectorAll('input.botonesConst').forEach(input => {
    input.style.setProperty('background', '#5a0e0e', 'important');
    input.style.setProperty('color', '#f0d9a0', 'important');
});
    ...
    button2.classList.add("flashLight", "btn", "mt-2");
    button2.style.minWidth= "100px";
  ...
  if (formConst) {
    formConst.addEventListener("submit", function (event) {
      ...
      if (numeroIngresado == numeroCorrecto) {
        elemento.className = "resultCorrect";

        elemento.innerHTML = 'A ding is heard, the floor starts to rumble and a new passage is <a class="links" href="./LORE/Constellation_secret.html">open</a>';
        // Aquí podrías redirigir a otro sitio si es correcto:
        // window.location.href = "https://tupagina.com/enlace.html";
      } else {
        elemento.className = "resultWrong";
        elemento.textContent = "A loud buzz comes from the machine";
      }
    });
  }
